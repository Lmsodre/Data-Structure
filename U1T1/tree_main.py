...
class AVLTree(BST):
    ...
    # PRINT FUNCTION FOR TESTER PURPOSE (PEDRO VITOR)

    def print_tree(self):
        """Imprime a árvore AVL."""
        self._print_tree_recursive(self.root)

    def _print_tree_recursive(self, node, indent="", prefix="Root: "):
        """Função auxiliar para impressão recursiva da árvore."""
        if node is not None:
            print(indent + prefix + str(node.value) + " (Height: " + str(node.height) + ", Imbalance: " + str(node.imbalance))
            if node.left_child is not None or node.right_child is not None:
                ...
                if node.right_child is None:
                    ...
                    prefix_right = "├── "
                self._print_tree_recursive(node.right_child, indent + indent_increment, prefix_right)
                self._print_tree_recursive(node.left_child, indent + indent_increment, "└── ")
    ...
    def find_words_with_prefix(self, prefix):
        ...
        """
        words_with_prefix = []
        self._find_words_with_prefix_recursive(self.root, prefix, words_with_prefix)
        return words_with_prefix

    def _find_words_with_prefix_recursive(self, current_node, prefix, result):
        if current_node is None:
            return
        ...
        if prefix < current_node.value:
            self._find_words_with_prefix_recursive(current_node.left_child, prefix, result)
        self._find_words_with_prefix_recursive(current_node.right_child, prefix, result)
